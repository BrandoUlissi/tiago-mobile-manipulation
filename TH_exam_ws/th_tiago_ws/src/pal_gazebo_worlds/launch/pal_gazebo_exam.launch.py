...
def start_gzserver(context, group_number, *args, **kwargs):
    ...
    group_number = context.perform_substitution(group_number)

    #world_name = f"exam_{group_number}"
    world_name = "topic_highlight_world"
    #world_name = "topic_highlight_static_obj"
    ...
    params_file = PathJoinSubstitution(substitutions=[pkg_path, "config", "gazebo_params.yaml"])
...
def generate_launch_description():
    ...
    declare_world_name = DeclareLaunchArgument(
        "world_name", default_value="", description="Specify world name, we'll convert to full path"
    )

    declare_group_number = DeclareLaunchArgument(
        "group_number",
        default_value="",
    )
    ...
    start_gazebo_server_cmd = OpaqueFunction(function=start_gzserver, args=[group_number])
    ...
    ld.add_action(declare_group_number)
